...
GENERATED_ROLEPLAY_SCRIPTS: dict[str, tuple[CoachTurn, ...]] = {
    ...
    "a2-final-test-practice": (
        CoachTurn(coach="Let's do a quick final practice. Ready?", hint="Jawab siap.", sample_answer="Yes, ready.", focus="Start practice", expected_keywords=("yes",)),
        CoachTurn(coach="Invite a friend for coffee this weekend.", hint="Gunakan Do you want to...?", sample_answer="Do you want to grab coffee this weekend?", focus="Invitation", expected_keywords=("want to", "weekend")),
        CoachTurn(coach="Say a symptom and duration.", hint="Gunakan I've had ... for ...", sample_answer="I've had a cough for two days.", focus="Symptom + duration", expected_keywords=("have had", "for")),
    ),
    ...
    "b1-final-conversation": (
        ...
        CoachTurn(coach="Now state your goal with a deadline.", hint="My goal is to... by ...", sample_answer="My goal is to speak more confidently by next month.", focus="Goal with deadline", expected_keywords=("goal", "by")),
        CoachTurn(coach="Share one problem and your solution, then preference + next step.", hint="Sometimes..., so... I prefer... because... So my next step is...", sample_answer="Sometimes I get distracted, so I leave my phone in another room. I prefer Conversation Coach because it gives feedback. So my next step is to practice three times this week.", focus="Problem + preference + next step", expected_keywords=("so", "prefer", "because", "next step")),
    ),
    ...
    "b2-final-discussion": (
        CoachTurn(coach="Start by framing the scope and success criteria.", hint="Let's define the scope... what does success look like?", sample_answer="Great. Let's define the scope and what success looks like.", focus="Frame", expected_keywords=("scope", "success")),
        CoachTurn(coach="Leadership wants new features too. Handle concerns and trade-offs.", hint="I understand the concern... The trade-off is...", sample_answer="I understand the concern. The trade-off is speed versus reliability, so we should keep scope tight.", focus="Concerns + trade-offs", expected_keywords=("understand", "trade-off")),
        CoachTurn(coach="Make a recommendation and confirm next steps with timeline.", hint="Given these constraints... Next steps are... today... by Friday.", sample_answer="Given these constraints, I'd recommend a time-boxed pilot. Next steps are: I'll share a plan today, and you'll review it by Friday.", focus="Decision + next steps", expected_keywords=("recommend", "next steps", "by")),
    ),
    ...
    "c1-final-conversation": (
        CoachTurn(coach="Frame the conversation goal.", hint="Let me frame this...", sample_answer="Let me frame this: we need reliable outcomes and stakeholder confidence before scaling.", focus="Frame", expected_keywords=("frame", "confidence")),
        CoachTurn(coach="Respond with nuance and catch implied concerns.", hint="On balance... What I'm hearing is... do you mean X or Y?", sample_answer="On balance, we can move fast, but only if we limit scope and validate metrics. What I'm hearing is some fear of visibility. Do you mean scope concerns or accountability concerns?", focus="Nuance + listening", expected_keywords=("on balance", "only if", "hearing")),
        CoachTurn(coach="Close with a decision and next steps.", hint="The decision is... Next steps are... today... tomorrow...", sample_answer="The decision is to run a two-week pilot with clear success criteria. Next steps are: I'll send a one-page plan today, and we'll align stakeholders tomorrow.", focus="Decision + next steps", expected_keywords=("decision", "next steps", "today")),
    ),
    ...
    "checking-in": (
        ...
        CoachTurn(coach="Sure. Do you have a reservation?", hint="Jawab dengan under + name.", sample_answer="Yes. I have a reservation under Mina Kim.", focus="Confirm reservation name", expected_keywords=("reservation", "under")),
    ...
    "clear-argument-mission": (
        CoachTurn(coach="State your position.", hint="Mulai dengan In my view...", sample_answer="In my view, we should move routine updates to a written summary.", focus="Position", expected_keywords=("in my view", "should")),
        CoachTurn(coach="Support it with two reasons.", hint="One reason is... Another reason is...", sample_answer="One reason is it saves time. Another reason is it improves focus.", focus="Reasons", expected_keywords=("one reason", "another reason")),
        CoachTurn(coach="Give an example, then respond to my concern and propose next steps.", hint="For example... That's a fair point. However... Let's...", sample_answer="For example, last month we spent 30 minutes repeating updates. That's a fair point. However, we can keep one monthly live session. Let's run a one-month trial and review the results.", focus="Example + counterpoint + next steps", expected_keywords=("for example", "fair point", "however", "trial")),
    ),
    "client-conversation-mission": (
        CoachTurn(coach="Start by understanding our needs.", hint="Just to clarify... biggest pain point...", sample_answer="Just to clarify, who is the main user group and what's the biggest pain point today?", focus="Discovery", expected_keywords=("clarify", "pain point")),
        CoachTurn(coach="Now explain two options and a trade-off, then recommend one.", hint="We have two options... The trade-off is... I'd recommend...", sample_answer="We have two options: a quick fix this week, or a more robust solution in two weeks. The trade-off is speed versus stability. I'd recommend the robust option if the timeline allows.", focus="Options + recommendation", expected_keywords=("two options", "trade-off", "recommend")),
        CoachTurn(coach="We are concerned about disruption. Handle it and confirm next steps.", hint="I understand the concern... To reduce the risk... Next steps are... Does that timeline work...?", sample_answer="I understand the concern. To reduce the risk, we can run a two-week pilot and send weekly summaries. Next steps are: I'll send a draft plan today, and you'll review it by Friday. Does that timeline work for you?", focus="Concerns + next steps", expected_keywords=("understand", "reduce", "next steps", "timeline")),
    ),
    ...
    "confirming-details": (
        ...
        CoachTurn(coach="Yes, that's right.", hint="Sebutkan nama dan eja.", sample_answer="My name is Mina Park. P-A-R-K.", focus="Give name and spelling", expected_keywords=("my name is", "p-a-r-k")),
    ...
    "information-discussion-mission": (
        CoachTurn(coach="Summarize the article briefly.", hint="The article is about... The main point is...", sample_answer="The article is about remote work and productivity. The main point is clear rules improve focus.", focus="Summary", expected_keywords=("article is about", "main point")),
        CoachTurn(coach="Is the source reliable? Explain why and what you'd check.", hint="I'm not sure it's reliable because... I'd check...", sample_answer="I'm not sure it's reliable because it doesn't cite data. I'd check official reports or reputable outlets.", focus="Reliability", expected_keywords=("reliable", "because", "I'd check")),
        CoachTurn(coach="Now respond to new contradictory information and propose next steps.", hint="I wasn't aware... That changes things... I'd like to understand... Can you share...?", sample_answer="I wasn't aware of that. That changes things. I'd like to understand the methodology. Can you share the link so we can revise our conclusion?", focus="New info + next steps", expected_keywords=("aware", "changes", "understand", "share")),
    ),
    "invitation-mission": (
    ...
    "leadership-coaching-mission": (
        CoachTurn(coach="Set direction and ask for options.", hint="The direction I'd like to set is... What options do you see?", sample_answer="The direction I'd like to set is stabilizing billing first. What options do you see right now?", focus="Direction + options", expected_keywords=("direction", "options")),
        CoachTurn(coach="Coach them toward a small next step and success criteria.", hint="What would success look like? smallest next step?", sample_answer="What would success look like for the pilot, and what's the smallest next step you can take today?", focus="Coach", expected_keywords=("success", "smallest next step")),
        CoachTurn(coach="Recommend an option and confirm agreement.", hint="Given our constraints... Can we agree on...?", sample_answer="Given our constraints, I'd recommend a phased rollout. Can we agree on scope today and review metrics weekly?", focus="Decision", expected_keywords=("constraints", "recommend", "agree")),
    ),
    "making-a-proposal": (
    ...
    "meeting-participation-mission": (
        CoachTurn(coach="Open a topic for today's meeting.", hint="I'd like to bring up...", sample_answer="I'd like to bring up the timeline for the next release.", focus="Open topic", expected_keywords=("bring up", "timeline")),
        CoachTurn(coach="Clarify scope and confirm what is out of scope.", hint="Just to clarify... Does this include...? out of scope...", sample_answer="Just to clarify, does this include the admin dashboard changes? If not, that's out of scope for now.", focus="Clarify scope", expected_keywords=("clarify", "include", "out of scope")),
        CoachTurn(coach="Give one feedback point and then summarize action items.", hint="Overall... I noticed... To summarize... Action items are...", sample_answer="Overall it was clear, but I noticed the intro was long. To summarize, action items are: I'll update the tickets, and you'll confirm the design handoff.", focus="Feedback + summary", expected_keywords=("noticed", "to summarize", "action items")),
    ),
    "negotiation-mission": (
    ...
    "opening-a-meeting-point": (
        ...
        CoachTurn(coach="Invite input from the team.", hint="Gunakan I'd like to hear your thoughts.", sample_answer="I'd like to hear your thoughts first, then we can decide.", focus="Invite input", expected_keywords=("hear", "thoughts")),
    ),
    ...
    "problem-solving-discussion-mission": (
        CoachTurn(coach="Start by framing the problem and scope.", hint="Let's define... Can we agree on the scope?", sample_answer="Let's define the problem statement first. Can we agree on the scope?", focus="Framing", expected_keywords=("define", "scope")),
        CoachTurn(coach="Now explain one possible cause with evidence language.", hint="Based on the data... one possible cause is...", sample_answer="Based on the data, one possible cause is confusion in checkout after the redesign.", focus="Causes", expected_keywords=("based on", "data", "possible")),
        CoachTurn(coach="Discuss trade-offs and recommend a next step with mitigation.", hint="The trade-off is... Given these constraints... We can mitigate it by...", sample_answer="The trade-off is speed versus reliability. Given these constraints, I'd recommend a two-week pilot. We can mitigate risk by setting clear metrics and a strict timeline.", focus="Trade-offs + recommendation", expected_keywords=("trade-off", "recommend", "mitigate")),
    ),
    ...
    "responding-to-long-turns": (
        CoachTurn(coach="Summarize what you heard.", hint="Let me make sure I got this...", sample_answer="Let me make sure I got this: we need to manage dependencies, time pressure, and stakeholder visibility.", focus="Summary", expected_keywords=("make sure", "got this")),
        CoachTurn(coach="Extract the key points.", hint="The key points are...", sample_answer="The key points are: keep changes small, communicate clearly, and monitor risk.", focus="Key points", expected_keywords=("key points",)),
        CoachTurn(coach="Give a structured response.", hint="Here's how I'd respond...", sample_answer="Here's how I'd respond: propose phases, share metrics, and set weekly check-ins.", focus="Response", expected_keywords=("here's how", "phases", "weekly")),
    ),
    "responding-to-new-information": (
        ...
        CoachTurn(coach="Agree on next steps.", hint="We should check... Can you share... Then we can revise...", sample_answer="We should check the full report. Can you share the link? Then we can revise our conclusion.", focus="Next steps", expected_keywords=("check", "share", "revise")),
    ),
    ...
    "review-nuance-and-strategy": (
        CoachTurn(coach="Give a nuanced position with a condition.", hint="On balance... but I'd...", sample_answer="On balance, I support it, but I'd adjust the scope to reduce risk.", focus="Nuance", expected_keywords=("on balance", "scope", "risk")),
    ...
    "review-presenting-and-debate": (
        ...
        CoachTurn(coach="Respond under pressure with if/then logic and summarize.", hint="If we accept..., then... In short...", sample_answer="If we accept speed as the priority, then we should time-box a pilot and limit scope. In short: validate with metrics, then expand safely.", focus="Pressure response", expected_keywords=("if", "then", "in short")),
    ),
    ...
    "strategic-workplace-mission": (
        CoachTurn(coach="Start by aligning priorities and constraints.", hint="To make sure we're aligned... top priority... biggest constraint...", sample_answer="To make sure we're aligned, what's your top priority and biggest constraint?", focus="Align", expected_keywords=("aligned", "priority", "constraint")),
        CoachTurn(coach="Set realistic expectations and timeline with assumptions.", hint="What we can commit to is... assuming...", sample_answer="What we can commit to is a smaller release by next week, assuming no new blockers.", focus="Expectations", expected_keywords=("commit", "assuming")),
        CoachTurn(coach="Communicate the main risk and mitigation steps.", hint="The main risk is... We can mitigate it by...", sample_answer="The main risk is incidents during rollout. We can mitigate it by time-boxing the rollout, adding monitoring, and having a rollback plan.", focus="Risk", expected_keywords=("main risk", "mitigate", "rollback")),
    ),
    ...
    "summarizing-decisions": (
        CoachTurn(coach="Summarize the key decision.", hint="Gunakan To summarize...", sample_answer="To summarize, we agreed to keep scope limited to the core feature.", focus="Summarize decision", expected_keywords=("to summarize", "agreed")),
        CoachTurn(coach="List action items with owners.", hint="Action items are: I'll..., you'll...", sample_answer="Action items are: I'll update the tickets, and you'll confirm the design handoff.", focus="Action items", expected_keywords=("action items", "I'll", "you'll")),
        CoachTurn(coach="Set a next checkpoint.", hint="We'll check progress on ...", sample_answer="We'll check progress on Thursday.", focus="Next checkpoint", expected_keywords=("check progress",)),
    ),
    "summarizing-what-you-heard": (
        CoachTurn(coach="Start with a summary of the facts and concerns.", hint="So, to summarize...", sample_answer="So, to summarize: the timeline is tight and clients are sensitive to change.", focus="Summary", expected_keywords=("to summarize", "timeline")),
        CoachTurn(coach="State the decision and open questions.", hint="The decision is... The open questions are...", sample_answer="The decision is to propose a phased rollout with clear metrics. The open questions are around resourcing and internal alignment.", focus="Decision + open questions", expected_keywords=("decision", "open questions")),
        CoachTurn(coach="Confirm your summary with the group.", hint="Just to confirm... Does that capture it...?", sample_answer="Just to confirm, I'll send the summary and ask if anything is missing. Does that capture it accurately?", focus="Confirm", expected_keywords=("confirm", "capture")),
    ),
    ...
    "travel-situation-mission": (
        ...
        CoachTurn(coach="Okay. Do you have a reservation?", hint="Jawab dengan under + name.", sample_answer="Yes. I have a reservation under Mina Kim.", focus="Confirm reservation", expected_keywords=("reservation", "under")),
