...
async def generate_partner_reply(
    *,
    ...
    try:
        ...
        logger.error("partner_reply_llm_failed topic=%s level=%s: %s", topic.key, level_code, exc)
        return fallback
    except Exception as exc:  # never break a paid turn
        logger.error("partner_reply_unexpected_error topic=%s level=%s: %s", topic.key, level_code, exc)
        return fallback
...
def _reply_system_prompt(topic: PartnerTopic, level_code: str, turns_left: int) -> str:
    ...
    return (
        f"You are {topic.partner_role} named {topic.partner_name}. You are having a short "
        "spoken role-play conversation with an Indonesian English learner to help them practice.\n"
        ...
        "steer back to the topic in character. Never start a new unrelated topic.\n"
        "- Keep your reply natural for your role but matched to the learner's level. One short turn.\n"
        "- Speak ONLY as the character. Do not give meta feedback, grammar corrections, or notes.\n"
...
def _fallback_reply(*, topic: PartnerTopic, turns_left: int) -> PartnerReply:
    ...
    return PartnerReply(
        reply="That's interesting! Let's keep going. Can you tell me a bit more?",
        on_topic=True,
        should_end=False,
    )
